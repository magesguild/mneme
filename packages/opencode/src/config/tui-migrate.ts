...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from Mneme config files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, Brand.configStem),
    ...(await Filesystem.findUp([Brand.configFile, Brand.configFileJsonc], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, Brand.configStem))
  }
