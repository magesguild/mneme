...
// etc.).
//
// The visual language mirrors the Mneme app with a small self-contained token
// set and a generic mark until the final visual identity is selected.
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      headline: "Authorization successful",
       message: provider ? `Mneme is now connected to ${escapeHtml(provider)}.` : "Mneme is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
         ? `Mneme couldn't finish connecting to ${escapeHtml(provider)}.`
         : "Mneme couldn't complete authorization.",
      detail,
       footnote: "Close this window and try again from Mneme.",
    }),
  })
}
...
<html lang="en">
  <head>
   ...
   <title>${escapeHtml(input.title)} · Mneme</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  var card=document.getElementById("oc-card"),headline=document.getElementById("oc-headline"),message=document.getElementById("oc-message"),detail=document.getElementById("oc-detail"),footnote=document.getElementById("oc-footnote");
   function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("Mneme couldn't finish connecting to "+PROVIDER+"."):"Mneme couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from Mneme."}
   function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("Mneme is now connected to "+PROVIDER+"."):"Mneme is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// Generic placeholder mark until Mneme's final visual identity is selected.
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" fill="none" aria-label="Mneme" role="img">
        <rect x="4" y="4" width="40" height="40" rx="8" stroke="var(--oc-icon-base)" stroke-width="4" />
        <path d="M16 24h16M24 16v16" stroke="var(--oc-icon-strong)" stroke-width="4" stroke-linecap="round" />
      </svg>`
