...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `mneme models` to list available models",
      "Or check your config (mneme.json) provider/model names",
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, Mneme does not support MCP authentication yet.`
  }
  return undefined
}
